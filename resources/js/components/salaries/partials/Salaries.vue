        ...
        <v-card-text class="mt-1">
          <v-row>
            <v-col cols="12">
              <table class="w-100" id="salries_table">
                ...
                <tbody class="text-center">
                  <template v-for="(salary, i) in salaries">
                    <tr :key="salary.id">
                      <td>{{ i + 1 }}</td>
                      <td>{{ salary.month_formatted }}</td>
                      <td>{{ money(salary.additional_amount) }}</td>
                      <td>{{ money(salary.deducted_amount) }}</td>
                      <td>{{ money(salary.total_paid) }}</td>
                      <td>{{ salary.date }}</td>
                      <td>{{ money(salary.balance) }}</td>
                      <td>
                        <v-chip :color="getStatusType(salary.status)" x-small>
                          {{ salary.status }}
                        </v-chip>
                      </td>
              ...
              <!-- Add payment dialog -->
              <v-dialog v-model="addPaymentDialog" max-width="600" persistent>
                <AddPayment
