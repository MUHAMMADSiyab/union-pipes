...
export default {
  ...
  methods: {
    ...mapActions({
      ...
      updateNozzle: "nozzle/updateNozzle",
    }),

    async update() {
      this.formLoading = true;

      await this.updateNozzle(this.data);
  ...
  async mounted() {
    const nozzle_id = this.$route.params.id;

    await Promise.all([this.getDispensers(), this.getNozzle(nozzle_id)]);

    if (!this.nozzle) {
      return this.$router.push({ name: "not_found" });
    }

    const { id, name, dispenser_id, code, description } = this.nozzle;
